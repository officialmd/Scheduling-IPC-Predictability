/*
latency_test_inheritance.c

latency vs message size experiment with priority inheritance enabled.

same as latency_test.c but with inheritance turned on. the server
channel is created with ChannelCreate(0) instead of
ChannelCreate(_NTO_CHF_FIXED_PRIORITY), so the server inherits the
client's priority when handling a request.

we compare results against the no-inheritance version to see if
inheritance adds any measurable overhead to ipc round-trip time
...
static void *server_thread(void *arg) {
    ...
    // inheritance enabled (no fixed priority flag)
    ...
    printf("[server] started at priority %d, channel %d (inheritance enabled)\n",
...
static void print_results(void) {
    printf("\n========== latency vs message size (inheritance) ==========\n");
    ...
    printf("\ncompare against the no-inheritance results.\n");
    printf("key things to check:\n");
    printf("  - does inheritance add measurable overhead?\n");
    printf("  - is the latency trend shape the same?\n");
    printf("  - compare min values between the two runs\n");
}

int main(void) {
    printf("=== latency vs message size (with inheritance) ===\n");
