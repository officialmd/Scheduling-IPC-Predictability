/*
qnet_distribution_test.c

Distributed round-trip latency experiment for QNX/QNET, aligned with the
paper's distributed communication study (Setting C style validation).

What this file is for:
- measure round-trip message latency between two QNX nodes over QNET
- compare message sizes from 96 B to 2048 B
- keep the server fixed-priority (no inheritance)
- produce a clean table of min / avg / max latency

How to use it:
1) build this file on both QNX nodes
2) start the server on the remote node:
      qnet_distribution_test server
   it prints its PID and CHID
3) on the client node, run:
      qnet_distribution_test client <server_node_name> <server_pid> <server_chid>
   example:
      qnet_distribution_test client node2 123456 7

Notes:
- this version uses ChannelCreate(_NTO_CHF_FIXED_PRIORITY), so the server
  keeps its own priority while serving requests.
- the client connects to the remote node using netmgr_strtond() and
  ConnectAttach(), which lets the same MsgSend/MsgReceive/MsgReply flow work
  across nodes through QNET.
*/
...
static int run_server(void) {
    ...
    printf("[server] pid=%d chid=%d priority=%d inheritance=disabled\n",
...
static int run_client(const char *node_name, pid_t server_pid, int server_chid) {
    ...
    printf("\n========== qnet distributed latency (no inheritance) ==========\n");
